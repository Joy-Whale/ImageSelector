...
public class CropOverlayView extends View implements CropPhotoViewAttacher.IGetImageBounds {
	...
	void init() {
		...
		mBorderPaint = PaintUtil.newBorderPaint(getContext().getResources());
		mBorderPaint.setColor(mBorderPaintColor);
		mGuidelinePaint = PaintUtil.newGuidelinePaint(getContext().getResources());
		clipPath = new Path();
	}

	/**
	 * Set the shape of the cover to cover
	 * @param shape shape TypeID
	 */
